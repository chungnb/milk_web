<x-app-layout>
    <section class="content">
        <div class="container mx-auto p-6">
            ...
            <form action="{{ route('admin.cate.update', $cate->id) }}" method="POST" enctype="multipart/form-data"
                class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $cate->name) }}"
                        class="mt-1 p-2 border border-gray-300 rounded-md w-full" required>
                </div>

                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700">Hình ảnh</label>
                    <input type="file" name="image" id="image" value="{{ old('image', $cate->image) }}"
