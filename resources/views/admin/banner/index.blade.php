            ...
            <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
                ...
                <tbody>
                    ...
                    <tr class="border-t hover:bg-gray-50">
                        <td class="py-2 px-4">{{ $banner->id }}</td>
                        <td class="py-2 px-4">{{ $banner->title }}</td>
                        <td class="py-2 px-4">@if($banner->image)
                            <img src="{{ asset('storage/' . $banner->image) }}" alt="{{ $banner->title }}" width="100">
                            @else
                            No Image
                            @endif
                        </td>
                        <td class="py-2 px-4">{{ $banner->created_at->format('Y-m-d') }}</td>
                        <td class="py-2 px-4">
